                          ...
                          int iterations, double eta)
{
    ...
    for (int i = 0; i < batches; i++) {
        auto batch = vector<vector<unsigned char>>(begin(data) + i * batchSize, begin(data) + (i+1) * batchSize);
        auto batchLabel = std::vector<std::array<unsigned char, 10>>(begin(labels) + i * batchSize, begin(labels) + (i+1) * batchSize);

        trainBatch(batch, batchLabel, iterations, eta, i+1, correctRate);
...
void NeuralNetwork::trainBatch(const std::vector<std::vector<unsigned char>> &data,
               const std::vector<std::array<unsigned char, 10>> &labels,
               int iterations, double eta, int batchNum, double correctRate) {
    int n = static_cast<int>(data.size());
    for (int t = 0; t < iterations; t++)
    {
        for (int i = 0; i < n; i++)
        {
                ...
                gradients.push_back(out[out.size() - 1].get_mul(-out[out.size() - 1] + 1).get_mul(labelData - out[out.size() - 1]));

                for (int k = 1; k < layers - 1; k++)
                {
                    auto gradient = sigmoidPrime(in[in.size() - k - 1]);
                    gradient += weights[weights.size() - k].get_transpose() * gradients[k - 1];
                    gradients.push_back(gradient);
                }

                for (int k = 0; k < layers - 1; k++)
                {
                    auto delta = gradients[k] * (out[out.size() - k - 2].get_transpose()) * eta;
                    deltas.push_back(delta);
                    weights[weights.size() - 1 - k] += delta;
                }
        }

            cout << "\rTrained iteration "
                << t + 1 << " in batch " << batchNum << ". Correct rate: "
                << correctRate << "%." << flush;
    }
}
